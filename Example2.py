# Задача 2. Задан массив из случайных цифр на 15 элементов.
#  На вход подаётся трёхзначное натуральное число. 
# Напишите программу, которая определяет, есть в массиве последовательность из трёх элементов,
#  совпадающая с введённым числом.
# [0, 5, 6, 2, 7, 7, 8, 1, 1, 9] - 277 -> да
# [4, 4, 3, 6, 7, 0, 8, 5, 1, 2] - 812 -> нет

import random
def result():
    numbers = [random.randint(1,9) for i in range(15)]
    Numberx = (input("Введите трёхзначное число: "))
    count=0
    for i in range(0,len(numbers)-3):
        if int(Numberx[count])==numbers[i] and numbers[i+1]==int(Numberx[count+1]) and numbers[i+2]==int(Numberx[count+2]):
            return print(f"{numbers} - {Numberx}-> yes")
            break
    return print(f"{numbers} - {Numberx}-> no")
result()
